...
- **Documentation**: Add `.md` or `.mdx` files to the `docs/` directory - they appear automatically in sidebars
- **Blog Posts**: Add files to `blog/` directory with date prefix (YYYY-MM-DD-title.md)
- **Navigation**: Uses autogenerated sidebars with intelligent sorting (index files first, then alphabetical)
- **Manual Ordering**: Use `sidebar_position` frontmatter or see [Manual Ordering Guide](.claude/docs/MANUAL_ORDERING.md)
- **Homepage**: Customize `src/pages/index.tsx` and `src/components/HomepageFeatures/`

### Autogenerated Sidebar Benefits
- **Add/remove files freely** - No manual sidebar maintenance required
- **Intelligent sorting** - Index files appear first, others alphabetically
- **Graceful handling** - Missing files don't break the build
- **Custom sorting available** - See guides for manual control when needed
...
- **🚨 CRITICAL CSS RULE: Never use `!important` statements as workarounds. The current CSS has 49 `!important` statements that need systematic removal using proper CSS specificity and ELEVATE token hierarchy. Always research the Docusaurus-native approach first, then use ELEVATE token extensions if needed.**
...
## Claude Documentation References

**Important**: All Claude-specific implementation guides and notes are stored in `.claude/docs/`. Reference these documents for detailed information about systems and processes:

### Content Management & Automation
- **[Autogenerated Sidebars Guide](.claude/docs/AUTOGEN_GUIDE.md)** - Complete guide to Docusaurus autogenerated sidebars with custom sorting
- **[Manual Ordering Guide](.claude/docs/MANUAL_ORDERING.md)** - How to force manual order for specific folders while keeping auto-generation
- **[Component Changelog System](.claude/docs/COMPONENT-CHANGELOG-SYSTEM.md)** - Automated changelog generation and management for ELEVATE components

### ELEVATE Design Token Integration
- **[ELEVATE Token Regime Memory](.claude/docs/ELEVATE-TOKEN-REGIME-MEMORY.md)** - Critical memory instructions for maintaining ELEVATE Design Token integration and ComponentShowcase functionality
- **[CSS Token Clarification Process](.claude/docs/CSS-TOKEN-CLARIFICATION-PROCESS.md)** - Systematic process for converting CSS to use ELEVATE tokens while avoiding `!important` statements

### Documentation Organization Pattern
When creating new Claude-specific documentation:
1. **Create files in `.claude/docs/`** - Keep Claude implementation notes separate from user documentation
2. **Use descriptive UPPERCASE names** - Follow pattern: `SYSTEM-NAME-GUIDE.md`
3. **Reference in CLAUDE.md** - Add links in appropriate sections
4. **Include practical examples** - Always provide code examples and usage patterns

This keeps Claude's implementation knowledge organized and easily accessible for future sessions.